...
#         input_ids_copy[:,self.num_trigger_tokens + 1:] = 0
...
#             # images_embeds = model.get_image_features(random_noise)
...
# #             text_embeds = model.get_text_features(input_ids=input_ids_copy, attention_mask=attention_mask)
# #             images_embeds = model.get_image_features(noise[index].to(options.device))
# #             loss2 = attacks.sim_loss(text_embeds,images_embeds.data)
# #             loss2.backward()
# #             grad2 = extracted_grads[0]
# #             # print(grad2)
...
# #                                                 increase_loss=False, num_candidates=128,
# #                                                 num_trigger_tokens=num_trigger_tokens)
# #             optimize_trigger_token = utils.get_best_candidates_attention(model,
...
#             logging.info(attacks.decode_vis(input_ids[0]))
#             candidates = attacks.hotflip_attack(grad, embedding_weight,
#                                                 increase_loss=True, num_candidates=64,
#                                                 num_trigger_tokens=num_trigger_tokens)
#             optimize_trigger_token = utils.get_best_candidates(model,
#                                                                  input_ids, attention_mask, pixel_values,
#                                                                  trigger_token_ids[index],
#                                                                  candidates, num_trigger_tokens,criterion=criterion,
#                                                                  increase_loss=True,options=options)
...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    for _ in range(0,10):
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            # global extracted_grads
            extracted_grads = []
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, pixel_values=noise[index].to(options.device))
            loss, _, _ = get_loss(model, outputs, criterion, options,gather_backdoor_indices=None)
            logging.info("*"*60)
            logging.info(f"Text Loss: {loss}")
            loss.backward()
            ...
            logging.info(attacks.decode_vis(input_ids[0]))
            candidates = attacks.hotflip_attack(grad, embedding_weight,
                                                increase_loss=False, num_candidates=32,
                                                num_trigger_tokens=num_trigger_tokens)
            optimize_trigger_token = utils.get_best_candidates(model,
                                                                 input_ids, attention_mask, pixel_values,
                                                                 trigger_token_ids[index],
