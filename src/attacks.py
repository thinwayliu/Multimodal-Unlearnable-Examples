...
def decode2(input_ids):
    decode_list = []
    transform = Tokenizer()
    for text in input_ids:
        d_text = transform.decode2(text.cpu().tolist())
        decode_list.append(d_text)
    return decode_list
...
def hotflip_attack_new(averaged_grad, embedding_matrix,
    ...
    gradient_dot_embedding_matrix = torch.matmul(averaged_grad.unsqueeze(1),embedding_matrix.transpose(1, 2)).squeeze(1)
