...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    noise = torch.zeros([class_num, 3, 224, 224])
    #设定伪标签，为了减少shortcut
    #利用文本的token聚类去选择标签
#     feature = []
...
#         feature_index.append(index)
    ...
    index_list = torch.randint(class_num,size=(data_samples_num,))
    ...
    trigger_token_ids = torch.randint(30522,size=(class_num,options.token_num))
    ...
    for text_time in range(0,20):  # optimize theta for M steps
        optimizer.zero_grad()
        model.train()
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            if text_time <5:
                ...
                candidates = attacks.hotflip_attack(grad, embedding_weight,
                                                    increase_loss=False, num_candidates=8,
            ...
            # random_class = torch.randint(high=class_num, size=(input_ids.shape[0],))  
            for i, _ in enumerate(batch["pixel_values"]):
