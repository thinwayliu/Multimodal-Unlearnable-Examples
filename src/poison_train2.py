...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    for _ in range(0,6):  # optimize theta for M steps
        optimizer.zero_grad()
        model.train()
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            normal_embeds = attacks.normal_embeds(input_ids,attention_mask,model,options)
            model.zero_grad()
            ...
            candidates = attacks.hotflip_attack(grad, embedding_weight,
                                                increase_loss=False, num_candidates=64,
