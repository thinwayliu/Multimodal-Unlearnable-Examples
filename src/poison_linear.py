...
class PerturbationTool():
    ...
    def min_min_attack(self, batch, model, criterion, batch_token,random_noise=None, options=None):
        ...
        text_idx = attacks.pred_text_idx(model,input_ids,attention_mask,options).data
        
        if torch.sum(random_noise) == 0:
            random_noise = (torch.randint(-8, 8, [input_ids.shape[0], 3, 224, 224], dtype=torch.int)/255-mean)/var
            random_noise = random_noise.to(options.device)
        ...
        for _ in range(self.num_steps):
            ...
            linear_loss = attacks.linear_loss(perturb_img-pixel_values.data,text_idx)
            loss += 0.5*linear_loss
