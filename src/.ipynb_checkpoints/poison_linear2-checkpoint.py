...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    for _ in range(0,15):  # optimize theta for M steps
        optimizer.zero_grad()
        model.train()
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, pixel_values=pixel_values)
            loss, _, _ = get_loss(model, outputs, criterion, options,gather_backdoor_indices=None)
            loss2 = attacks.linear_loss(noise[index],model,input_ids,attention_mask,options)
            print(loss2)
            loss = loss2
            ...
            with torch.no_grad():
                outputs = model(input_ids=input_ids, attention_mask=attention_mask, pixel_values=pixel_values)
                loss, _, _ = get_loss(model, outputs, criterion, options, gather_backdoor_indices=None)
                loss = attacks.linear_loss(noise[index], model, input_ids, attention_mask, options,is_test=True)
