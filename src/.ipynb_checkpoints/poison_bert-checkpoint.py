...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    for _ in range(0,10):  # optimize theta for M steps
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            loss = text_loss(model, input_ids, attention_mask, pixel_values, pred_img_idx, criterion, options)
            ...
            num_candidates = 256
            cand_index = attacks.hotflip_attack_new(grad, embedding_weight,
                                                    increase_loss=False, num_trigger_tokens=num_trigger_tokens,
                                                    num_candidates = num_candidates) #[32,3,256]
            ...
            for sen_idx in range(input_ids.shape[0]):
                for token_index in range(1,1+options.token_num):
                    for cand_idx in range(num_candidates):
                        with torch.no_grad():
                            input_ids_copy = input_ids.clone()
                            input_ids_copy[sen_idx, token_index] = cand_index[sen_idx,token_index,cand_idx]
                            loss_cand = text_loss(model, input_ids, attention_mask, pixel_values,
                                                  pred_img_idx, criterion, options)
                            if loss_cand < loss:
                                loss = loss_cand
            ...
            # test loss
                with torch.no_grad():
                    ...
                    loss_cand = text_loss(model, input_ids_copy, attention_mask, pixel_values, pred_img_idx, criterion, options)
                    logging.info(f"Text Loss cand: {loss_cand}")
            ...
            print(input_ids[0])
            with torch.no_grad():
                loss = text_loss1(model, grad, input_ids, attention_mask, pixel_values, pred_img_idx, criterion, options)
