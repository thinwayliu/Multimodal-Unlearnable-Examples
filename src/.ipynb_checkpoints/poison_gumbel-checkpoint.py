...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    noise = torch.zeros([500, 3, 224, 224])
    index_list = torch.randint(500,size=(30000,))
    torch.save(index_list, os.path.join(options.log_dir_path, "index_list.pt"))
    ...
    trigger_token_ids = torch.randint(30522,size=(500,options.token_num))
    ...
    for _ in range(0,10):  # optimize theta for M steps
        ...
        for j in range(0, 40):
            ...
            with torch.no_grad():
                pixel_values = clamp(batch["pixel_values"] + noise[index_list[index]], lower_limit, upper_limit)
                for i, _ in enumerate(batch["pixel_values"]):
                    input_ids[i, 1:1 + num_trigger_tokens] = trigger_token_ids[index_list[index[i]]]
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            tokenizer = BertTokenizer.from_pretrained(
                r'C:\Users\Administrator\.cache\huggingface\hub\models--bert-base-uncased\snapshots\1dbc166cf8765166998eff31ade2eb64c8a40076')
            ...
            loss_ori = loss
            cache_trigger_token = torch.zeros_like(trigger_token_ids[index_list[index],:])

            for token_index in range(1,1+options.token_num):
                predict_list = attacks.predict_text(process_text,token_index,tokenizer,bert,options,num_cand=50)
                ...
                cache_trigger_token[:, token_index - 1] = selected_values.squeeze(1).cpu()

            # test loss
            with torch.no_grad():
                input_ids_copy[:, 1:1 + num_trigger_tokens] = cache_trigger_token
                outputs = model(input_ids=input_ids_copy.to(options.device), attention_mask=attention_mask, pixel_values=pixel_values)
                loss_cand, _, _ = get_loss(model, outputs, criterion, options, gather_backdoor_indices=None)
                if loss_cand < loss_ori:
                    trigger_token_ids[index_list[index]] = cache_trigger_token

            # candidates = attacks.hotflip_attack(grad, embedding_weight,
            #                                     increase_loss=False, num_candidates=3,
            #                                     num_trigger_tokens=num_trigger_tokens)
            # optimize_trigger_token = utils.get_best_candidates(model,
            #                                                      input_ids, attention_mask, pixel_values,
            #                                                      trigger_token_ids[index_list[index]],
            #                                                      candidates, num_trigger_tokens,criterion=criterion,
            #                                                      increase_loss=False,options=options)

            # trigger_token_ids[index_list[index]] = optimize_trigger_token
            input_ids[:, 1:1 + num_trigger_tokens] = trigger_token_ids[index_list[index]]
            ...
            for i, _ in enumerate(batch["pixel_values"]):
                ...
                batch_noise.append(noise[index_list[index[i]]])
                batch_token.append(trigger_token_ids[index_list[index[i]]])
                idx += 1
            ...
            for i, delta in enumerate(eta):
                noise[index_list[index[i]]] = delta.clone().detach().cpu()
