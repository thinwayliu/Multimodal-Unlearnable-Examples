...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    def text_loss1(model,input_ids,attention_mask,pixel_values,pred_idx,criterion,options,balance_para=1):
        outputs = model(input_ids=input_ids, attention_mask=attention_mask, pixel_values=pixel_values)
    ...
    for text_t in range(0,20):
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            for token_index in range(1,1+options.token_num):
                decode_text = attacks.decode_text(input_ids_copy,options.token_num)
            # print(decode_text)
                process_text = attacks.process_text(decode_text, options, tokenizer)
                predict_list = attacks.predict_text(process_text,token_index,tokenizer,bert,options,num_cand=5)
                ...
                # print(selected_values.shape) #[128,4]
                # print(cand_word.shape) #[128,2000]
                selected_values = torch.gather(cand_word, 1, cand_index.to(options.device))
                ...
                for cand in range(num_candidates):
                    with torch.no_grad():
                        # print(selected_values[:,cand])
                        input_ids_copy[:, token_index] = selected_values[:,cand].cpu()
                        outputs = model(input_ids=input_ids_copy,
                                        attention_mask=attention_mask, pixel_values=pixel_values)
                        loss_cand, _, _ = get_loss(model, outputs, 
                                                   criterion, options,          gather_backdoor_indices=None)
                        logging.info(f"Text Loss cand: {loss_cand}")

                    if loss_cand < loss_ori:
                        trigger_token_ids[index,token_index-1] = selected_values[:,cand].cpu()
                        loss_ori = loss_cand
                        # input_ids_copy[:, token_index] = trigger_token_ids[:,token_index-1]
                    
                    
                input_ids_copy[:, token_index] = trigger_token_ids[index,token_index-1]
            ...
            with torch.no_grad():
                outputs = model(input_ids=input_ids,
                                attention_mask=attention_mask, pixel_values=pixel_values)
                loss, _, _ = get_loss(model, outputs, 
                                            criterion, options,          gather_backdoor_indices=None)
