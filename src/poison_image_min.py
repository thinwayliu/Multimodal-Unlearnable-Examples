...
def poison(epoch, model, data, optimizer, scheduler, scaler, options,processor):
    ...
    for _ in range(0,1):  # optimize theta for M steps
        optimizer.zero_grad()
        model.train()
...
#             scaler.update()
        ...
        for _, (batch,index) in tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            normal_embeds = attacks.normal_embeds(input_ids,attention_mask,model,options).data
            ...
            optimize_trigger_token = utils.get_best_candidates_max(model,
                                                                 input_ids, attention_mask, pixel_values,
                                                                 trigger_token_ids[index],
                                                                 candidates, num_trigger_tokens,criterion=criterion,
                                                                 increase_loss=True,options=options,normal_embeds=normal_embeds)
