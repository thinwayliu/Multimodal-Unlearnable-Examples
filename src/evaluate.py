...
def get_face_metrics(model, processor, test_dataloader, options):
    ...
    (r1, r5, r10, medr, meanr), (r1i, r5i, r10i, medri, meanri) = i2t_t2i(image_embeddings.cpu(), text_embeddings.cpu())
...
def get_zeroshot_metrics2(model, processor, test_dataloader, options):
    logging.info("Started zeroshot testing")
    if options.checkpoint_finetune is not None:
        if(os.path.isfile(options.checkpoint_finetune)):
            checkpoint = torch.load(options.checkpoint_finetune, map_location = options.device)
    ...
    if options.eval_data_type is not None:
        ...
        embeddings = torch.cat((image_embeddings[:50],text_embeddings[:50]),dim=0)
        ...
        # logging.info(embeddings_tsne)
        
        np.save('./mme-tsne.npy', embeddings_tsne)
...
def i2t_t2i(images, captions, npts=None):
    """
    Images->Text (Image Annotation) and Text->Images (Image Retrieval)
    ...
    i2t_medr = np.floor(np.median(ranks_i2t)) + 1
    i2t_meanr = ranks_i2t.mean() + 1

    # t2i
    ranks_t2i = np.zeros(npts)
    score_matrix = np.dot(images, captions.T)
    for i in range(npts):
        cur_rank = np.argsort(score_matrix[i])[::-1]
        for index, j in enumerate(cur_rank):
            if j == i:
    ...
    t2i_medr = np.floor(np.median(ranks_t2i)) + 1
    t2i_meanr = ranks_t2i.mean() + 1
    ...
    if(options.master):
        ...
        if(data["eval_test"] is not None): 
            ...
            if options.face is not None:
                print("*"*100)
                metrics.update(get_face_metrics(model, processor, data["eval_test"], options))
            else:
