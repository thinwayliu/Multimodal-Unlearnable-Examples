...
class CLIP(nn.Module):
    ...
    # def get_text_features1(self, input_ids = None, attention_mask = None,origin_input_ids=None):
    #     x = input_ids.type(self.dtype)  # [batch_size, n_ctx, d_model]
    #
    #     x = x + self.positional_embedding.type(self.dtype)
    #     x = x.permute(1, 0, 2)  # NLD -> LND
    #     x = self.transformer(x)
    ...
    #     # x.shape = [batch_size, n_ctx, transformer.width]
    #     # take features from the eot embedding (eot_token is the highest number in each sequence)
    #     x = x[torch.arange(x.shape[0]), origin_input_ids.argmax(dim=-1)] @ self.text_projection
    #
    #     return x
    ...
    # def test(self, input_ids, origin_input_ids, attention_mask, pixel_values):
    #     image_features = self.get_image_features(pixel_values = pixel_values)
    #     text_features = self.get_text_features1(input_ids = input_ids, attention_mask = attention_mask,origin_input_ids=origin_input_ids)
    #
    #     image_features = image_features / image_features.norm(dim=-1, keepdim=True)
    #     text_features = text_features / text_features.norm(dim=-1, keepdim=True)
    #
